...
Самая заметная зона для дальнейшего улучшения — довести архитектуру до более цельного вида в духе `UNITY_COMPOSITION_GUIDE.md`:

- меньше legacy-кода;
- меньше глобальных классов;
- меньше прямого `Input` и runtime `Find/GetComponent`;
- завершить заготовленные сервисы;
- дочистить неиспользуемые и недоведённые окна/системы.
