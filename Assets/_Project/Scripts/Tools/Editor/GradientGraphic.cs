...
public class GradientGraphicEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        DrawPropertiesExcluding(serializedObject, "m_Script", "sprite", "m_Color", "topLeftColor", "topRightColor",
            "bottomLeftColor", "bottomRightColor", "gradientSmoothness", "useSlicedSprite", "fillCenter",
            "pixelsPerUnitMultiplier", "m_OnCullStateChanged");
...
namespace _Project.Scripts.Tools.Editor
{
    ...
    public class GradientGraphic : MaskableGraphic, ILayoutElement
    {
        ...
        [Range(1, 5)]
        [Tooltip(
            "Increasing this value will make the gradient smoother by generating more vertices for the UI mesh; increase it only when needed")]
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            if (useSlicedSprite && hasBorder)
            {
                ...
                for (int x = 0; x < 3; x++)
                {
                    ...
                    for (int y = 0; y < 3; y++)
                    {
                        ...
                        {
                            Color c1 = GetColorAt((s_SlicedVertices[x].x - bottomLeftX) * invWidth,
                                (s_SlicedVertices[y].y - bottomLeftY) * invHeight);

                            Color c2 = GetColorAt((s_SlicedVertices[x].x - bottomLeftX) * invWidth,
                                (s_SlicedVertices[y2].y - bottomLeftY) * invHeight);

                            Color c3 = GetColorAt((s_SlicedVertices[x2].x - bottomLeftX) * invWidth,
                                (s_SlicedVertices[y2].y - bottomLeftY) * invHeight);

                            Color c4 = GetColorAt((s_SlicedVertices[x2].x - bottomLeftX) * invWidth,
                                (s_SlicedVertices[y].y - bottomLeftY) * invHeight);

                            vh.AddVert(new Vector3(s_SlicedVertices[x].x, s_SlicedVertices[y].y, 0), c1,
                                new Vector2(s_SlicedUVs[x].x, s_SlicedUVs[y].y));

                            vh.AddVert(new Vector3(s_SlicedVertices[x].x, s_SlicedVertices[y2].y, 0), c2,
                                new Vector2(s_SlicedUVs[x].x, s_SlicedUVs[y2].y));

                            vh.AddVert(new Vector3(s_SlicedVertices[x2].x, s_SlicedVertices[y2].y, 0), c3,
                                new Vector2(s_SlicedUVs[x2].x, s_SlicedUVs[y2].y));

                            vh.AddVert(new Vector3(s_SlicedVertices[x2].x, s_SlicedVertices[y].y, 0), c4,
                                new Vector2(s_SlicedUVs[x2].x, s_SlicedUVs[y].y));
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i <= gradientSmoothness; i++)
                            {
                                for (int j = 0; j <= gradientSmoothness; j++)
                                {
                                    ...
                                    Vector3 _position = new Vector3(_bottomLeftX + _width * normalizedX,
                                        _bottomLeftY + _height * normalizedY, 0f);

                                    Vector2 _uv = new Vector2(_bottomLeftUVx + _uvWidth * normalizedX,
                                        _bottomLeftUVy + _uvHeight * normalizedY);

                                    Color _color = GetColorAt((_position.x - bottomLeftX) * invWidth,
                                        (_position.y - bottomLeftY) * invHeight);
            ...
            else
            {
                ...
                if (gradientSmoothness <= 1)
                {
                    ...
                    vh.AddVert(new Vector3(bottomLeftX + width, bottomLeftY + height, 0f), topRightColor,
                        new Vector2(uv.z, uv.w));

                    vh.AddVert(new Vector3(bottomLeftX + width, bottomLeftY, 0f), bottomRightColor,
                ...
                else
                {
                    ...
                    for (int i = 0; i <= gradientSmoothness; i++)
                    {
                        for (int j = 0; j <= gradientSmoothness; j++)
                        {
                            ...
                            Vector3 _position = new Vector3(bottomLeftX + width * normalizedX,
                                bottomLeftY + height * normalizedY, 0f);

                            Vector2 _uv = new Vector2(uv.z * normalizedX + uv.x * (1f - normalizedX),
                                uv.w * normalizedY + uv.y * (1f - normalizedY));
