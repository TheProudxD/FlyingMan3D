...
namespace _Project.Scripts.Infrastructure.Services.Logger
{
    public class FileWriter : IDisposable
    {
        ...
        private void ManagePath()
        {
            ...
            _filePath = $"{_folder}/{_prevDate.ToString(DATE_FORMAT)}.log";
        }
        ...
        public void Write(LogMessage message)
        {
            try
            {
                if (message.Message.Length > MAX_MESSAGE_LENGTH)
                {
                    ...
                    _messages.Enqueue(
                        new LogMessage($"Message is too long {message.Message.Length}. Preview: {preview}",
                            type: message.Type)
                        {
                            Time = message.Time
                        });
            ...
            while (!_disposing)
            {
                while (!_messages.IsEmpty)
                {
                    try
                    {
                        ...
                        string messageToWrite = string.Format(LOG_TIME_FORMAT, message.Time,
